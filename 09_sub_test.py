...
out_dir = "out_"+sys.argv[0].split(".")[0]
if(not os.path.exists(out_dir)):
    os.makedirs(out_dir)
out_name = os.path.join(out_dir, "trackml_submit_{0}.csv".format(sys.argv[0].split(".")[0]))
print(out_name)

submission_list = []
for (event_id, hits) in tqdm(load_dataset(path_to_input, parts=["hits"]), total=125):

    path_to_sub = get_path_to_submission("out_08_merge_test", event_id)
